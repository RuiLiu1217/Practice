...
{
    TABLE = std::vector<std::vector<int>>(m, std::vector<int>(n, -1));
    return uniquePaths_impl(m - 1, n - 1);
}
...
int LeetCode::_0062_UniquePaths::uniquePaths_impl(int m, int n)
{
    ...
    if(m == 0 || n == 0) {
        // the first row / column will have only one way to achieve
        return TABLE[m][n] = 1; 
    }
    int u = uniquePaths_impl(m-1, n);
    int l = uniquePaths_impl(m, n-1);
    return TABLE[m][n] = u + l;
}
