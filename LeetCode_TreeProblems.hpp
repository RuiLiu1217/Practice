...
/*
Given the root of a binary tree, return the sum of every tree node's tilt. The tilt of a tree 
node is the absolute difference between the sum of all left subtree node values and all right 
subtree node values. If a node does not have a left child, then the sum of the left subtree 
node values is treated as 0. The rule is similar if there the node does not have a right child.
...
Tile of node 1 : |2-3| = 1 (left subtree is just left child, so sum is 2; right subtree is just 
...
private:
    int tilt = 0;
    int sum(TreeNode* root) { // 有迷惑性的一点是算的对象并不当作返回值，而是在计算过程中更新
        if(!root) {
            return 0;
        }
        int l = sum(root->left);
...
private:
    ...
    void fill(TreeNode* root, std::vector<std::vector<std::string>>& res, int h, int l, int r) {
        if(!root) {
            return;
        }
        int mid = l + (r - l) / 2;
...
一道变态题目
! Hint: Heap + numbering Offset
*/
class _0662_MaximumWithOfBinaryTree {
private:
    std::vector<long> ids;
public:
...
private:
    long dfs(TreeNode* root, int depth, int id) {
        if(root == nullptr) {
            return 0;
        }
        if(ids.size() == depth) { // 没有理解为什么 看花花的视频
            ids.push_back(id);
        }
        return std::max({
            id - ids[depth] + 1,
...
public:
    ...
    std::pair<int, TreeNode*> depth(TreeNode* root);
};
...
public:
    ...
    void deepestLeavesSum(TreeNode* root, int depth) {
        ...
        if(root->left == nullptr && root->right == nullptr) {
            if(depth > maxDepth) { // 替换法，如果遇到更深的，之前算的都不作数了，
                val = root->val;
                maxDepth = depth;
            } else if(depth == maxDepth) { // 如果遇到一样深的，那就加上当前的结果.
                val += root->val;
            }
        }
...
private:
    std::unordered_set<long> numSet;
public:
    int maxProduct(TreeNode* root) {
        long totsum = totalSum(root);
        long best = 0;
        for(auto& n : numSet) {
            best = std::max(best, (totsum - n) * n);
        }
    ...
    long totalSum(TreeNode* root) {
        ...
        long totSum = leftSum + rightSum + root->val;
        numSet.insert(totSum);
        return totSum;
    }
};
