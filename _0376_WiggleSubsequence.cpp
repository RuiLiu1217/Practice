...
int LeetCode::_0376_WiggleSubsequence::wiggleMaxLength(std::vector<int>& nums) {
    int size = nums.size();
    if (size == 0) {
        return 0;
    }
    ...
    */
    ...
    for(int i = 1; i < size; ++i) {
        if(nums[i] > nums[i-1]) {
             ...
             * (2) We can replace N with nums[i-1] (we still have a valid subsequence of type D since 
             * N >= nums[i] > nums[i-1] i.e. N > nums[i-1]), and then add nums[i] to the subsequence, 
             * and we have a longer subsequence of type U. Therefore up[i] = down[i-1] + 1
             ...
             * But this means that we could have used nums[i-1] already to make a longer subsequence of 
             * type D. So even if we can use nums[i], there is no gain in using it, so we keep the old 
             * value of down (down[i] = down[i-1])
             *
             */
            up[i] = down[i-1] + 1;
            down[i] = down[i-1];
